...
CDM_INLINE void put_bits(Block &b, uint32_t bit, uint32_t count,
                         uint64_t value) {
  if (bit < 64) {
    b.low |= value << bit;
    if (bit + count > 64)
...
CDM_INLINE Block pack_block(const uint8_t ep8[2][4],
...
template <bool Srgb> CDM_INLINE Float4 rgba8_to_linear(const uint8_t v[4]) {
  if constexpr (Srgb)
    return {bc1::c_srgb_to_linear[v[0]], bc1::c_srgb_to_linear[v[1]],
            bc1::c_srgb_to_linear[v[2]], v[3] * (1.0f / 255.0f)};
  return {v[0] * (1.0f / 255.0f), v[1] * (1.0f / 255.0f),
          v[2] * (1.0f / 255.0f), v[3] * (1.0f / 255.0f)};
}

template <bool Srgb>
CDM_INLINE void palette(const uint8_t ep[2][4], Float4 out[16]) {
...
template <bool Srgb>
CDM_INLINE void quadrant_means(const Block &block, Float4 out[4]) {
  ...
  for (uint32_t q = 0; q < 4; ++q) {
    ...
    out[q] = (pal[sel[y * 4 + x]] + pal[sel[y * 4 + x + 1]] +
              pal[sel[(y + 1) * 4 + x]] + pal[sel[(y + 1) * 4 + x + 1]]) *
             0.25f;
  }
}
...
template <bool Srgb>
CDM_INLINE uint8_t quantize_component(float target, uint32_t pbit, bool alpha) {
  uint32_t best = pbit;
  float error = 1e30f;
  for (uint32_t q = 0; q < 128; ++q) {
    uint32_t code = (q << 1) | pbit;
    float value =
        (Srgb && !alpha) ? bc1::c_srgb_to_linear[code] : code * (1.0f / 255.0f);
...
template <bool Srgb>
CDM_INLINE void quantize_endpoint(const Float4 &v, uint8_t out[4]) {
  float best = 1e30f;
  for (uint32_t p = 0; p < 2; ++p) {
    uint8_t q[4] = {quantize_component<Srgb>(v.r, p, false),
                    quantize_component<Srgb>(v.g, p, false),
                    quantize_component<Srgb>(v.b, p, false),
                    quantize_component<Srgb>(v.a, p, true)};
...
CDM_INLINE Block generate_child(const Block &p00, const Block &p10,
                                const Block &p01, const Block &p11, bool srgb,
                                Float4 parent_means[4], uint32_t valid_width,
  ...
  for (int p = 0; p < 4; ++p)
    parent_means[p] =
        (s[p * 4] + s[p * 4 + 1] + s[p * 4 + 2] + s[p * 4 + 3]) * 0.25f;
...
CDM_INLINE Block generate_from_means(const MeanImage &source, uint32_t bx,
                                     uint32_t by, bool srgb, uint32_t vw,
                                     uint32_t vh) {
  Float4 s[16];
  for (uint32_t i = 0; i < 16; ++i) {
    uint32_t t = texel_index(i);
    s[i] = source.get(repeat_small_coordinate(bx * 4 + (t & 3), vw),
...
CDM_INLINE Float4x4 principal_axis_x4(const SymMat4x4 &cov) {
  ...
  v4f scale = _mm_max_ps(_mm_max_ps(abs4(cov.rr).v, abs4(cov.gg).v),
                         _mm_max_ps(abs4(cov.bb).v, abs4(cov.aa).v));
  scale = _mm_max_ps(scale.v,
                     _mm_max_ps(_mm_max_ps(abs4(cov.rg).v, abs4(cov.rb).v),
                                _mm_max_ps(abs4(cov.ra).v, abs4(cov.gb).v)));
  scale = _mm_max_ps(scale.v, _mm_max_ps(abs4(cov.ga).v, abs4(cov.ba).v));
  __m128 valid = _mm_and_ps(_mm_cmpgt_ps(scale.v, _mm_setzero_ps()),
                            _mm_cmple_ps(scale.v, _mm_set1_ps(FLT_MAX)));
  v4f inv_scale =
      _mm_div_ps(_mm_set1_ps(1), _mm_blendv_ps(_mm_set1_ps(1), scale.v, valid));
  Float4x4 axis = {cov.rr * inv_scale, cov.rg * inv_scale, cov.rb * inv_scale,
                   cov.ra * inv_scale};
  ...
  select({cov.rg * inv_scale, cov.gg * inv_scale, cov.gb * inv_scale,
          cov.ga * inv_scale});
  select({cov.rb * inv_scale, cov.gb * inv_scale, cov.bb * inv_scale,
          cov.ba * inv_scale});
  select({cov.ra * inv_scale, cov.ga * inv_scale, cov.ba * inv_scale,
          cov.aa * inv_scale});
...
template <bool Srgb>
CDM_INLINE Block encode_with_endpoints(const Float4 samples[16], Float4 p0,
                                       Float4 p1) {
  uint8_t ep[2][4];
...
template <bool Srgb>
CDM_INLINE void encode_samples_x4(const Float4 scalar[4][16], Block out[4]) {
  Float4x4 samples[16];
  for (int i = 0; i < 16; ++i) {
    samples[i].r = _mm_set_ps(scalar[3][i].r, scalar[2][i].r, scalar[1][i].r,
                              scalar[0][i].r);
    samples[i].g = _mm_set_ps(scalar[3][i].g, scalar[2][i].g, scalar[1][i].g,
                              scalar[0][i].g);
    samples[i].b = _mm_set_ps(scalar[3][i].b, scalar[2][i].b, scalar[1][i].b,
                              scalar[0][i].b);
    samples[i].a = _mm_set_ps(scalar[3][i].a, scalar[2][i].a, scalar[1][i].a,
                              scalar[0][i].a);
  }
  ...
  for (int lane = 0; lane < 4; ++lane)
    out[lane] = encode_with_endpoints<Srgb>(
        scalar[lane],
        {values[0][lane], values[1][lane], values[2][lane], values[3][lane]},
        {values[4][lane], values[5][lane], values[6][lane], values[7][lane]});
}

CDM_INLINE void generate_from_means_x4(const MeanImage &m, uint32_t bx,
                                       uint32_t by, uint32_t lanes, Block *out,
  ...
  for (uint32_t lane = 0; lane < lanes; ++lane)
    for (uint32_t i = 0; i < 16; ++i) {
      uint32_t t = texel_index(i);
      s[lane][i] =
          m.get(repeat_small_coordinate((bx + lane) * 4 + (t & 3), vw),
                repeat_small_coordinate(by * 4 + (t >> 2), vh));
    }
...
CDM_INLINE void generate_child_x4(const Block *row0, const Block *row1,
                                  Block *out, const MeanImage &means,
                                  uint32_t source_x, uint32_t y0, uint32_t y1,
                                  bool srgb, uint32_t vw, uint32_t vh) {
  Float4 s[4][16];
  for (uint32_t lane = 0; lane < 4; ++lane) {
    ...
    for (uint32_t p = 0; p < 4; ++p) {
      Float4 mean = (s[lane][p * 4] + s[lane][p * 4 + 1] + s[lane][p * 4 + 2] +
                     s[lane][p * 4 + 3]) *
                    .25f;
...
template <bool Srgb>
__device__ __forceinline__ Float4 device_color(const uint8_t endpoints[2][4],
                                               uint32_t selector) {
  uint8_t value[4];
  for (int channel = 0; channel < 4; ++channel)
    value[channel] =
        interpolate(endpoints[0][channel], endpoints[1][channel], selector);
  return {decode_channel<Srgb>(value[0]),
          decode_channel<Srgb>(value[1]),
          decode_channel<Srgb>(value[2]),
          value[3] * (1.0f / 255.0f)};
}

template <bool Srgb>
__device__ __forceinline__ Float4 device_quadrant_mean(const Block &block,
                                                       uint32_t quadrant) {
...
template <bool Srgb>
__device__ __forceinline__ uint8_t device_quantize(float target, uint32_t pbit,
                                                   bool alpha) {
  uint32_t best = pbit;
  ...
  for (uint32_t q = 0; q < 128; ++q) {
    ...
    const float value =
        Srgb && !alpha ? g_srgb_to_linear[code] : code * (1.0f / 255.0f);
...
template <bool Srgb>
__device__ __forceinline__ void device_quantize_endpoint(Float4 value,
                                                         uint8_t output[4]) {
  float best = 1e30f;
  for (uint32_t pbit = 0; pbit < 2; ++pbit) {
    uint8_t candidate[4] = {device_quantize<Srgb>(value.r, pbit, false),
                            device_quantize<Srgb>(value.g, pbit, false),
                            device_quantize<Srgb>(value.b, pbit, false),
                            device_quantize<Srgb>(value.a, pbit, true)};
    Float4 decoded = {decode_channel<Srgb>(candidate[0]),
                      decode_channel<Srgb>(candidate[1]),
                      decode_channel<Srgb>(candidate[2]),
                      candidate[3] * (1.0f / 255.0f)};
...
template <bool Srgb>
__device__ __forceinline__ void encode_half_warp(Float4 sample, uint32_t lane,
                                                 Block *output) {
  const unsigned mask = half_warp_mask();
  Float4 mean = {sum16(sample.r, mask) / 16,
                 sum16(sample.g, mask) / 16,
                 sum16(sample.b, mask) / 16,
                 sum16(sample.a, mask) / 16};
  Float4 delta = sample - mean;
  SymMat4 covariance = {sum16(delta.r * delta.r, mask) / 16,
                        sum16(delta.g * delta.g, mask) / 16,
                        sum16(delta.b * delta.b, mask) / 16,
                        sum16(delta.a * delta.a, mask) / 16,
                        sum16(delta.r * delta.g, mask) / 16,
                        sum16(delta.r * delta.b, mask) / 16,
                        sum16(delta.r * delta.a, mask) / 16,
                        sum16(delta.g * delta.b, mask) / 16,
                        sum16(delta.g * delta.a, mask) / 16,
                        sum16(delta.b * delta.a, mask) / 16};
  Float4 axis = {1, 0, 0, 0};
  if (lane == 0)
  ...
  const float low = min16(projection, mask),
              high = max16(projection, mask);
  Float4 endpoints_float[2] = {mean + axis * low, mean + axis * high};
  uint32_t endpoints[2][4] = {};
  if (lane == 0) {
    for (Float4 &v : endpoints_float)
      v = {fminf(1, fmaxf(0, v.r)), fminf(1, fmaxf(0, v.g)),
           fminf(1, fmaxf(0, v.b)), fminf(1, fmaxf(0, v.a))};
  ...
  if (lane == 0) {
    ...
    for (int i = 1; i < 16; ++i)
      selectors[texel_index(i)] =
          uint8_t(__shfl_sync(mask, selector, i, 16));
