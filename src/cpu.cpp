...
void downsample_scalar(const float *source, uint32_t source_width,
                       uint32_t source_height, float *destination,
                       uint32_t destination_width, uint32_t begin,
                       uint32_t end) {
...
void downsample_simd(const float *source, uint32_t source_width,
                     uint32_t source_height, float *destination,
  ...
  for (uint32_t y = begin; y < end; ++y) {
    ...
    for (; x + 3 < destination_width && x * 2 + 7 < source_width; x += 4) {
      ...
      const __m128 top =
          _mm_hadd_ps(_mm_loadu_ps(row0 + sx), _mm_loadu_ps(row0 + sx + 4));
      const __m128 bottom =
          _mm_hadd_ps(_mm_loadu_ps(row1 + sx), _mm_loadu_ps(row1 + sx + 4));
      _mm_storeu_ps(output + x,
                    _mm_mul_ps(_mm_add_ps(top, bottom), _mm_set1_ps(0.25f)));
    }
...
public:
  ...
  void parallel_rows(uint32_t rows, size_t work_items, Function function) {
    ...
    const uint32_t tasks =
        std::min<uint32_t>(rows, uint32_t(workers_.size()) + 1);
...
MeanImage make_mean_image(float *storage, size_t capacity, uint32_t width,
                          uint32_t height) {
...
template <bool Simd, typename MeanImage>
void downsample_means(const MeanImage &source, const MeanImage &destination) {
  ...
  auto rows = [&](uint32_t begin, uint32_t end) {
    for (uint32_t channel = 0; channel < MeanImage::channel_count; ++channel)
      downsample(source.channel(channel), source.width, source.height,
                 destination.channel(channel), destination.width, begin, end);
  };
  g_dispatcher.parallel_rows(destination.height,
                             size_t(destination.width) * destination.height *
                                 MeanImage::channel_count,
                             rows);
}
...
struct Bc1Codec {
  ...
  static Block child(const Block &a, const Block &b, const Block &c,
                     const Block &d, bool srgb, Color means[4], uint32_t w,
                     uint32_t h) {
    return bc1::generate_child_block_scalar(a, b, c, d, srgb, means, w, h);
  }
  static Block from_means(const MeanImage &m, uint32_t x, uint32_t y, bool srgb,
                          uint32_t w, uint32_t h) {
    return bc1::generate_child_block_from_means_scalar(m, x, y, srgb, w, h);
  }
  static void children_x4(const Block *r0, const Block *r1, Block *out,
                          const MeanImage &m, uint32_t x, uint32_t y0,
                          uint32_t y1, bool srgb, uint32_t w, uint32_t h) {
    bc1::generate_child_blocks_x4(r0, r1, out, srgb, m, x, y0, y1, w, h);
  }
  static void from_means_x4(const MeanImage &m, uint32_t x, uint32_t y,
                            uint32_t lanes, Block *out, bool srgb, uint32_t w,
                            uint32_t h) {
...
struct Bc6hCodec {
  ...
  using MeanImage = MeanImage3;
  static Block child(const Block &a, const Block &b, const Block &c,
                     const Block &d, bool, Color means[4], uint32_t w,
                     uint32_t h) {
    return bc6h::generate_child_block_scalar(a, b, c, d, means, w, h);
  }
  static Block from_means(const MeanImage &m, uint32_t x, uint32_t y, bool,
                          uint32_t w, uint32_t h) {
  ...
  static void from_means_x4(const MeanImage &m, uint32_t x, uint32_t y,
                            uint32_t lanes, Block *out, bool, uint32_t w,
                            uint32_t h) {
...
void process_rows(const Image *image, const MipLevel &previous,
                  const MipLevel &current, uint32_t level,
                  typename Codec::MeanImage &means, uint32_t begin,
                  uint32_t end) {
  using Block = typename Codec::Block;
  const Block *source =
      reinterpret_cast<const Block *>(image->data + previous.byte_offset);
  Block *destination =
      reinterpret_cast<Block *>(image->data + current.byte_offset);
  for (uint32_t by = begin; by < end; ++by) {
    ...
    if (level >= 2) {
      if constexpr (Simd)
        for (uint32_t bx = 0; bx < current.block_count_x; bx += 4)
          Codec::from_means_x4(
              means, bx, by, std::min(4u, current.block_count_x - bx),
              output + bx, image->is_srgb, current.width, current.height);
      else
    ...
    const uint32_t y0 = by * 2,
                   y1 = std::min(y0 + 1, previous.block_count_y - 1);
    const Block *row0 = source + size_t(y0) * previous.block_count_x;
    const Block *row1 = source + size_t(y1) * previous.block_count_x;
    uint32_t bx = 0;
    if constexpr (Simd)
      for (; bx + 3 < current.block_count_x &&
             (bx + 3) * 2 + 1 < previous.block_count_x;
           bx += 4)
        Codec::children_x4(row0 + bx * 2, row1 + bx * 2, output + bx, means,
                           bx * 2, y0, y1, image->is_srgb, current.width,
                           current.height);
    for (; bx < current.block_count_x; ++bx) {
      const uint32_t x0 = bx * 2,
                     x1 = std::min(x0 + 1, previous.block_count_x - 1);
      typename Codec::Color parent_means[4];
      output[bx] =
          Codec::child(row0[x0], row0[x1], row1[x0], row1[x1], image->is_srgb,
...
template <typename Codec, bool Simd> bool generate_cpu(Image *image) {
  ...
  const uint32_t bw = image->mips[0].block_count_x,
                 bh = image->mips[0].block_count_y;
  ...
  std::unique_ptr<float[]> base(
      new (std::nothrow) float[base_count * MeanImage::channel_count]);
  std::unique_ptr<float[]> scratch_data(
      new (std::nothrow) float[scratch_count * MeanImage::channel_count]);
  if (!base || !scratch_data)
    return false;
  MeanImage means = make_mean_image<MeanImage>(base.get(), base_count, bw, bh);
  MeanImage scratch =
      make_mean_image<MeanImage>(scratch_data.get(), scratch_count, sw, sh);
  for (uint32_t level = 1; level < image->mip_count; ++level) {
    ...
    auto rows = [image, previous, current, level, &means](uint32_t begin,
                                                          uint32_t end) {
      process_rows<Codec, Simd>(image, previous, current, level, means, begin,
                                end);
    };
    g_dispatcher.parallel_rows(
        current.block_count_y,
        size_t(current.block_count_x) * current.block_count_y, rows);
  }
  return true;
}
...
bool generate_mipmaps(Image *image, const Options &options) {
  ...
  if (image->format == Format::BC7) {
    const auto *blocks = reinterpret_cast<const bc7::Block *>(
        image->data + image->mips[0].byte_offset);
    const size_t count =
