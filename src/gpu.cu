...
static_assert(texel_index(0) == 0 && texel_index(3) == 5 &&
              texel_index(10) == 12 && texel_index(15) == 15);
...
generate_base_mip(const Block64 *source, uint32_t source_width,
                  uint32_t source_height, Block64 *destination,
                  uint32_t destination_width, uint32_t destination_height,
                  MeanImage means, uint32_t valid_width,
  ...
  Color sample = bc1::device_quadrant_mean<Srgb>(
      source[source_y * source_width + source_x], quadrant);
  Color parent_mean = {sum4(sample.r, mask) * 0.25f,
                       sum4(sample.g, mask) * 0.25f,
  ...
  if (valid_width < 4 || valid_height < 4) {
    uint32_t lane =
        repeat_small_sample(sample_index, valid_width, valid_height);
    sample = {__shfl_sync(mask, sample.r, lane, 16),
              __shfl_sync(mask, sample.g, lane, 16),
...
template <bool Srgb>
static __global__ void
generate_mean_mip(MeanImage source, Block64 *destination,
                  uint32_t destination_width, uint32_t destination_height,
                  MeanImage next_means, uint32_t valid_width,
                  uint32_t valid_height) {
  uint32_t sample_index = threadIdx.x & 15u;
  uint32_t output_index = blockIdx.x * (blockDim.x >> 4) + (threadIdx.x >> 4);
  uint32_t output_count = destination_width * destination_height;
  ...
  if (valid_width < 4 || valid_height < 4) {
    x = clamp_index(
        repeat_small_coordinate(output_x * 4 + (local & 3u), valid_width),
        source.width);
    y = clamp_index(
        repeat_small_coordinate(output_y * 4 + (local >> 2), valid_height),
        source.height);
...
static __global__ void
generate_bc6h_base_mip(const bc6h::Block *source, uint32_t source_width,
                       uint32_t source_height, bc6h::Block *destination,
                       uint32_t destination_width, uint32_t destination_height,
                       MeanImage means, uint32_t valid_width,
                       uint32_t valid_height) {
  uint32_t output_index = blockIdx.x * (blockDim.x >> 4) + (threadIdx.x >> 4);
  ...
  uint32_t x0 = clamp_index(ox * 2, source_width),
           x1 = clamp_index(x0 + 1, source_width);
  uint32_t y0 = clamp_index(oy * 2, source_height),
           y1 = clamp_index(y0 + 1, source_height);
  uint32_t sx = parent & 1u ? x1 : x0, sy = parent & 2u ? y1 : y0;
  Color sample =
      bc6h::device_quadrant_mean(source[sy * source_width + sx], quadrant);
  Color parent_mean = {sum4(sample.r, mask) * 0.25f,
                       sum4(sample.g, mask) * 0.25f,
                       sum4(sample.b, mask) * 0.25f};
  bool unique = parent == 0 || (parent == 1 && x1 != x0) ||
                (parent == 2 && y1 != y0) ||
                (parent == 3 && x1 != x0 && y1 != y0);
  ...
  if (valid_width < 4 || valid_height < 4) {
    uint32_t source_lane =
        repeat_small_sample(lane, valid_width, valid_height);
    sample = {__shfl_sync(mask, sample.r, source_lane, 16),
              __shfl_sync(mask, sample.g, source_lane, 16),
...
static __global__ void
generate_bc6h_mean_mip(MeanImage source, bc6h::Block *destination,
                       uint32_t destination_width, uint32_t destination_height,
                       MeanImage next_means, uint32_t valid_width,
                       uint32_t valid_height) {
  uint32_t lane = threadIdx.x & 15u;
  ...
  if (next_means.r && (lane & 3u) == 0) {
    uint32_t quadrant = lane >> 2, nx = ox * 2 + (quadrant & 1u),
             ny = oy * 2 + (quadrant >> 1);
  ...
  if (valid_width < 4 || valid_height < 4) {
    x = clamp_index(
        repeat_small_coordinate(ox * 4 + (local & 3u), valid_width),
        source.width);
    y = clamp_index(
        repeat_small_coordinate(oy * 4 + (local >> 2), valid_height),
        source.height);
    index = size_t(y) * source.width + x;
    sample = {source.r[index], source.g[index], source.b[index]};
  }
...
template <bool Srgb>
static __global__ void
generate_bc7_base(const bc7::Block *source, uint32_t source_width,
                  uint32_t source_height, bc7::Block *destination,
                  uint32_t destination_width, uint32_t destination_height,
                  bc7::MeanImage means, uint32_t valid_width,
                  uint32_t valid_height) {
  uint32_t output_index = blockIdx.x * (blockDim.x >> 4) + (threadIdx.x >> 4),
           lane = threadIdx.x & 15u;
  if (output_index >= destination_width * destination_height)
    return;
  unsigned mask = half_warp_mask();
  uint32_t ox = lane == 0 ? output_index % destination_width : 0,
  ...
  uint32_t x0 = clamp_index(ox * 2, source_width),
           x1 = clamp_index(x0 + 1, source_width),
           y0 = clamp_index(oy * 2, source_height),
           y1 = clamp_index(y0 + 1, source_height);
  uint32_t sx = (parent & 1) ? x1 : x0, sy = (parent & 2) ? y1 : y0;
  Color4 sample =
      bc7::device_quadrant_mean<Srgb>(source[sy * source_width + sx], quadrant);
  Color4 pm = {sum4(sample.r, mask) * .25f, sum4(sample.g, mask) * .25f,
               sum4(sample.b, mask) * .25f, sum4(sample.a, mask) * .25f};
  bool unique = parent == 0 || (parent == 1 && x1 != x0) ||
                (parent == 2 && y1 != y0) ||
                (parent == 3 && x1 != x0 && y1 != y0);
  if (quadrant == 0 && unique) {
  ...
  if (valid_width < 4 || valid_height < 4) {
    ...
    sample = {
        __shfl_sync(mask, sample.r, l, 16), __shfl_sync(mask, sample.g, l, 16),
...
template <bool Srgb>
static __global__ void
generate_bc7_mean(bc7::MeanImage source, bc7::Block *destination,
                  uint32_t destination_width, uint32_t destination_height,
                  bc7::MeanImage next, uint32_t valid_width,
                  uint32_t valid_height) {
  uint32_t oi = blockIdx.x * (blockDim.x >> 4) + (threadIdx.x >> 4),
           lane = threadIdx.x & 15u;
  if (oi >= destination_width * destination_height)
    return;
  unsigned mask = half_warp_mask();
  uint32_t ox = lane == 0 ? oi % destination_width : 0,
           oy = lane == 0 ? oi / destination_width : 0;
  ox = __shfl_sync(mask, ox, 0, 16);
  oy = __shfl_sync(mask, oy, 0, 16);
  uint32_t t = texel_index(lane);
  uint32_t x = clamp_index(ox * 4 + (t & 3), source.width),
           y = clamp_index(oy * 4 + (t >> 2), source.height);
  ...
  if (valid_width < 4 || valid_height < 4) {
    x = clamp_index(repeat_small_coordinate(ox * 4 + (t & 3), valid_width),
                    source.width);
    y = clamp_index(
        repeat_small_coordinate(oy * 4 + (t >> 2), valid_height),
        source.height);
...
static std::array<float, MaxValue> make_srgb_quantize_thresholds() {
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    uint32_t b = ((uint32_t(i) + 1) << (8 - bits)) |
                 ((uint32_t(i) + 1) >> (2 * bits - 8));
...
static bool initialize_tables() {
  ...
  static const std::array<float, 31> threshold31 =
      make_srgb_quantize_thresholds<31>();
  static const std::array<float, 63> threshold63 =
      make_srgb_quantize_thresholds<63>();
  initialized = cudaMemcpyToSymbol(g_srgb_to_linear, srgb.data(),
                                   sizeof(srgb)) == cudaSuccess &&
                cudaMemcpyToSymbol(g_threshold31, threshold31.data(),
                                   sizeof(threshold31)) == cudaSuccess &&
                cudaMemcpyToSymbol(g_threshold63, threshold63.data(),
                                   sizeof(threshold63)) == cudaSuccess;
  return initialized;
}
...
struct CudaWorkspace {
  ...
  bool prepare(const Image *image) {
    const size_t base_count =
        (size_t)image->mips[0].block_count_x * image->mips[0].block_count_y;
    const size_t scratch_count =
        (size_t)((image->mips[0].block_count_x + 1) / 2) *
        ((image->mips[0].block_count_y + 1) / 2);
    const size_t channels = image->format == Format::BC7 ? 4 : 3;
    return data.allocate(image->data_size) &&
           base.allocate(base_count * channels) &&
...
template <bool Srgb>
static bool generate_cuda_impl(Image *image, uint8_t *device_data,
                               MeanImage means, MeanImage scratch) {
  ...
  for (uint32_t level = 1; level < image->mip_count; ++level) {
    ...
    auto *destination =
        reinterpret_cast<Block64 *>(device_data + current.byte_offset);
    uint32_t output_count = current.block_count_x * current.block_count_y;
    constexpr uint32_t outputs_per_block = threads / 16;
    uint32_t blocks =
        (output_count + outputs_per_block - 1) / outputs_per_block;
    if (level == 1) {
      const auto *source =
          reinterpret_cast<const Block64 *>(device_data + previous.byte_offset);
      generate_base_mip<Srgb><<<blocks, threads>>>(
          source, previous.block_count_x, previous.block_count_y, destination,
          current.block_count_x, current.block_count_y, means, current.width,
          current.height);
    } else {
      ...
      generate_mean_mip<Srgb><<<blocks, threads>>>(
          means, destination, current.block_count_x, current.block_count_y,
...
static bool generate_bc6h_cuda_impl(Image *image, uint8_t *device_data,
                                    MeanImage means, MeanImage scratch) {
  constexpr uint32_t threads = 256;
  constexpr uint32_t outputs_per_block = threads / 16;
  for (uint32_t level = 1; level < image->mip_count; ++level) {
    ...
    const MipLevel &current = image->mips[level];
    auto *destination =
        reinterpret_cast<bc6h::Block *>(device_data + current.byte_offset);
    uint32_t output_count = current.block_count_x * current.block_count_y;
    uint32_t blocks =
        (output_count + outputs_per_block - 1) / outputs_per_block;
    if (level == 1) {
      const auto *source = reinterpret_cast<const bc6h::Block *>(
          device_data + previous.byte_offset);
      generate_bc6h_base_mip<<<blocks, threads>>>(
          source, previous.block_count_x, previous.block_count_y, destination,
          current.block_count_x, current.block_count_y, means, current.width,
          current.height);
    } else {
      ...
      generate_bc6h_mean_mip<<<blocks, threads>>>(
          means, destination, current.block_count_x, current.block_count_y,
          next_means, current.width, current.height);
      if (level + 1 < image->mip_count)
        std::swap(means, scratch);
...
template <bool Srgb>
static bool generate_bc7_cuda_impl(Image *image, uint8_t *device_data,
                                   bc7::MeanImage means,
                                   bc7::MeanImage scratch) {
  constexpr uint32_t threads = 256, outputs_per_block = threads / 16;
  for (uint32_t level = 1; level < image->mip_count; ++level) {
    const MipLevel &previous = image->mips[level - 1],
                   &current = image->mips[level];
    auto *destination =
        reinterpret_cast<bc7::Block *>(device_data + current.byte_offset);
    uint32_t count = current.block_count_x * current.block_count_y,
             blocks = (count + outputs_per_block - 1) / outputs_per_block;
    if (level == 1) {
      const auto *source = reinterpret_cast<const bc7::Block *>(
          device_data + previous.byte_offset);
      generate_bc7_base<Srgb><<<blocks, threads>>>(
          source, previous.block_count_x, previous.block_count_y, destination,
          current.block_count_x, current.block_count_y, means, current.width,
          current.height);
    } else {
      ...
      generate_bc7_mean<Srgb><<<blocks, threads>>>(
          means, destination, current.block_count_x, current.block_count_y,
          next, current.width, current.height);
      if (level + 1 < image->mip_count)
        std::swap(means, scratch);
    }
    if (cudaGetLastError() != cudaSuccess)
...
bool prepare_mipmaps_cuda(const Image *image) {
  return image && cudaFree(nullptr) == cudaSuccess && initialize_tables() &&
         g_workspace.prepare(image);
}
...
bool generate_mipmaps_cuda(Image *image) {
  ...
  if (cudaMemcpy(g_workspace.data.get(), image->data, image->mips[0].byte_size,
                 cudaMemcpyHostToDevice) != cudaSuccess)
    return false;

  MeanImage means = {
      g_workspace.base.get(), g_workspace.base.get() + base_count,
      g_workspace.base.get() + base_count * 2, base_width, base_height};
  MeanImage scratch = {g_workspace.scratch.get(),
  ...
  bool generated =
      image->format == Format::BC6H_UF16
          ? generate_bc6h_cuda_impl(image, g_workspace.data.get(), means,
                                    scratch)
          : (image->format == Format::BC7
                 ? (image->is_srgb
                        ? generate_bc7_cuda_impl<true>(
                              image, g_workspace.data.get(), means4, scratch4)
                        : generate_bc7_cuda_impl<false>(
                              image, g_workspace.data.get(), means4, scratch4))
                 : (image->is_srgb
                        ? generate_cuda_impl<true>(
                              image, g_workspace.data.get(), means, scratch)
                        : generate_cuda_impl<false>(
                              image, g_workspace.data.get(), means, scratch)));
  const size_t generated_offset = image->mips[1].byte_offset;
  return generated && cudaMemcpy(image->data + generated_offset,
                                 g_workspace.data.get() + generated_offset,
                                 image->data_size - generated_offset,
